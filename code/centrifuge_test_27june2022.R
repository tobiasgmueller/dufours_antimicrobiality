# i ran a test plate to see how centrifuging and filtering dufours glands
# of (frozen) andrena regularis impact the growth of E coli (S17-1)
...
d <- read.csv("input/centrifuge_test_27june2022.csv")
labels <- read.csv("input/centrifuge_test_27june2022_plate_setup.csv")
...
write.csv(gr,"formatted_OD_values/june27_2022_centrifugeplate", row.names = FALSE)

### grofit curve fitting ####


t<-read.csv("formatted_OD_values/june27_2022_centrifugeplate", header=F)
...
curves<- od_raw%>%
  filter(treatment == c("control","dufours"))%>%
  ggplot()+
  geom_point(aes(x=time, y=od, color=treatment))+
  facet_wrap(~microbe)
curves
ggsave(plot=curves, filename = "output/centrifuge_test_27june2022.png")
...
write.csv(grofit, "output/27june2022_centrifugetest_grofitresults.csv")
...
grofit<- read.csv("output/27june2022_centrifugetest_grofitresults.csv")
